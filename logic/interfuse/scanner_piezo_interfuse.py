"""
This file contains the Qudi Interfuse between Scanner Tilt interfuse and Piezo Positioner.
...
class ScannerPiezoInterfuse(GenericLogic, ConfocalScannerInterface, MotorInterface):
    """ This interfuse connects the piezo with the Scanner Tilt interfuse.

    Example config for copy-paste:

    scanner_piezo_interfuse:
        module.Class: 'interfuse.scanner_piezo_interfuse.ScannerPiezoInterfuse'
        connect:
            confocalscanner1: 'niusb_6210_slowcounter'
            piezopositioner: 'arduino_mega2560'
    """

    confocalscanner1 = Connector(interface='ConfocalScannerInterface')
    piezopositioner = Connector(interface='MotorInterface')
    ...
    def on_activate(self):
        ...
        self._motor_device = self.piezopositioner()
    ...
    def scanner_set_position(self, x=None, y=None, z=None, a=None):
        ...
        @return int: error code (0:OK, -1:error)
        """
        self._scanning_device.scanner_set_position(x, y, z, a)
        return self._motor_device.move_abs({'x': x, 'y': y, 'z': z, 'a': a})
    ...
    def scan_line(self, line_path=None, pixel_clock=False):
        ...
        line_points = len(line_path[0])
        number_of_channels = self._scanning_device.number_of_channels
        count_total = np.zeros((line_points, number_of_channels))

        time.sleep(0.05)                    # Mitigates too high counts that appear occasionally

        for point in range(line_points):
            self._motor_device.move_abs({'x': line_path[0][point],
                                         'y': line_path[1][point],
                                         'z': line_path[2][point],
                                         'a': line_path[3][point]})

            count_point = self._scanning_device.read_count_diff(1/self._scanning_device._clock_frequency)

            for channel in range(number_of_channels):
                count_total[point][channel] = count_point[channel]
    ...
    def move_rel(self, param_dict):
        return self._motor_device.move_rel()

    def move_abs(self, param_dict):
        return self._motor_device.move_abs()

    def abort(self):
        return self._motor_device.abort()

    def get_pos(self, param_list=None):
        return self._motor_device.get_pos()

    def get_status(self, param_list=None):
        pass

    def calibrate(self, param_list=None):
        return self._motor_device.calibrate()

    def get_velocity(self, param_list=None):
        return self._motor_device.get_velocity()

    def set_velocity(self, param_dict):
        return self._motor_device.set_velocity()
