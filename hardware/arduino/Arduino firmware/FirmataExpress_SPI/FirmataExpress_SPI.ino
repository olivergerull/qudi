/*
  This FirmataExpress sketch was extended by some SPI functions (see "// AddedForSPI" comments)
...
#define CS_PIN                      53    // AddedForSPI
#define triggerpin                  22    // AddedForSPI:  pin that triggers SPI communication; don't connect anything to this pin and the 20 pins after
...
void sendSpi(unsigned long bytecode)                         // AddedForSPI: from here
{
...
void loop()
{
  ...
  if (SPI_trigger == 1) {                           // if trigger pin = 1: SPI message from Python can be read
    ...
    for (int i=(triggerpin+1); i < (triggerpin+21); i++){
      ...
      if (pin_value == 1) {                         // if value = 1: set corresponding SPI message bit to 1
        ...
        serial_SPI_msg += ((int32_t)1 << bitshift); // moves 1 to postition in the SPI bit code and adds it to SPI bit code
      }
